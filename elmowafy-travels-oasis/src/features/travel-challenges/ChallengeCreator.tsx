...
import { useFamilyData } from '../family/useFamilyData';
...
export const ChallengeCreator: React.FC<ChallengeCreatorProps> = ({
  ...
  // State for family members with loading state
  const [isLoadingFamily, setIsLoadingFamily] = useState(true);
  
  // Fetch family members with error state
  const { familyData, error: familyError } = useFamilyData();
  
  // Show loading state while family data is being loaded
  if (isLoadingFamily) {
    return (
  ...
  // Update family members when data is loaded or errors
  useEffect(() => {
    let isMounted = true;
